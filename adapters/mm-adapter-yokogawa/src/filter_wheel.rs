...
impl CsuXFilterWheel {
    pub fn new(wheel_nr: u32) -> Self {
        ...
        let labels = (0..num_positions).map(|i| format!("Filter-{}", i)).collect();
        let mut props = PropertyMap::new();
        props.define_property("Port", PropertyValue::String("Undefined".into()), false).unwrap();
        Self {
            props, transport: None, initialized: false,
            wheel_nr, position: 0, num_positions, labels, gate_open: true,
        }
    }
...
impl Device for CsuXFilterWheel {
    fn name(&self) -> &str { "CsuX-FilterWheel" }
    fn description(&self) -> &str { "Yokogawa CSU-X1 filter wheel" }

    fn initialize(&mut self) -> MmResult<()> {
        if self.transport.is_none() { return Err(MmError::NotConnected); }
        let query = format!("FW_POS, {}, ?", self.wheel_nr);
        let resp = self.cmd(&query)?;
        // Response is "<pos>\rA" or just "<pos>".  Split on any whitespace / CR.
...
mod tests {
    ...
    fn initialize_reads_position() {
        let t = MockTransport::new().expect("FW_POS, 1, ?\r", "3\rA");
    ...
    #[test]
    fn wheel2() {
        let t = MockTransport::new().expect("FW_POS, 2, ?\r", "2\rA");
