/// CrestOptics X-Light spinning disk motor.
///
...
impl XLightSpinMotor {
    pub fn new() -> Self {
        let labels = vec!["Off (no spin)".to_string(), "On (spinning)".to_string()];
...
impl Default for XLightSpinMotor { fn default() -> Self { Self::new() } }

impl Device for XLightSpinMotor {
    fn name(&self) -> &str { "XLight-SpinMotor" }
    fn description(&self) -> &str { "CrestOptics X-Light spinning disk motor" }

    fn initialize(&mut self) -> MmResult<()> {
        if self.transport.is_none() { return Err(MmError::NotConnected); }
        let resp = self.cmd("rN")?;
